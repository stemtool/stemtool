...
class atomic_dpc(object):
    ...
    def show_BF_ADF(self, imsize=(20, 10)):
        ...
        plt.axis('off')
        at = mploff.AnchoredText('ADF-STEM', prop=dict(size=fontsize), 
        ...
        plt.gca().add_artist(scalebar)
        plt.axis('off')
        at = mploff.AnchoredText('Summed 4D-STEM', prop=dict(size=fontsize), 
                                 frameon=True, loc='lower left')
        at.patch.set_boxstyle("round, pad=0., rounding_size=0.2")
        plt.gca().add_artist(at)
        plt.tight_layout()
    ...
    def initial_dpc(self, imsize= (30, 15)):
        ...
        for ii in range(len(yy)):
            ...
            self.YCom[yy[ii], xx[ii]] = self.inverse*((np.sum(np.multiply(qq, pattern))/np.sum(pattern)) - self.beam_y)
            self.XCom[yy[ii], xx[ii]] = self.inverse*((np.sum(np.multiply(pp, pattern))/np.sum(pattern)) - self.beam_x)
        ...
        at= mploff.AnchoredText('Shift in X direction', 
        ...
        at= mploff.AnchoredText('Shift in Y direction',
                                prop= dict(size= fontsize), frameon= True, 
                                loc= 'upper left')
        at.patch.set_boxstyle('round, pad= 0., rounding_size= 0.2')
        ax2.add_artist(at)
    ...
    def correct_dpc(self, imsize= (30, 15)):
        ...
        for ii in range(2):
            ...
            sol1 = min_x - 90
            sol2 = min_x + 90
            chg_sums[int(2*ii)] = np.sum(st.dpc.charge_dpc(xdpcf, self.YCom, sol1)*self.data_adf)
            chg_sums[int(2*ii+1)] = np.sum(st.dpc.charge_dpc(xdpcf, self.YCom, sol2)*self.data_adf)
        ...
        self.XComC, self.YComC = st.dpc.pol2cart(rho_dpc, (phi_dpc - (self.angle*((np.pi)/180))))
        ...
        scalebar.color= 'k'
        ax1.add_artist(scalebar)
        at= mploff.AnchoredText('Corrected shift in X direction', 
                                prop= dict(size= fontsize), frameon= True,
                                loc= 'upper left')
        at.patch.set_boxstyle('round, pad= 0., rounding_size= 0.2')
        ax1.add_artist(at)
        ...
        scalebar.color= 'k'
        ax2.add_artist(scalebar)
        at= mploff.AnchoredText('Corrected shift in Y direction',
                                prop= dict(size= fontsize), frameon= True, 
                                loc= 'upper left')
        at.patch.set_boxstyle('round, pad= 0., rounding_size= 0.2')
        ax2.add_artist(at)
    ...
    def show_charge(self, imsize= (15, 15)):
        ...
        self.charge = (-1)*((np.gradient(XComV)[1] + np.gradient(YComV)[0])/(self.calib*(10**(-12))))
    ...
    def show_potential(self, imsize= (15, 15)):
        ...
        plt.gca().add_artist(scalebar)
        plt.axis('off')
        at = mploff.AnchoredText('Measured potential',
                                 prop=dict(size=fontsize), frameon=True,
                                 loc='lower left')
        at.patch.set_boxstyle("round, pad=0., rounding_size=0.2")
        plt.gca().add_artist(at)
    ...
    def plot_color_dpc(self, skip=2, portion= 7, imsize= (20, 10)):
        ...
        cutter = 1/portion
        cutstart = (np.round(np.asarray(self.XComC.shape) - (cutter*np.asarray(self.XComC.shape)))).astype(int)
        ypos, xpos = np.mgrid[0:self.YComC.shape[0], 0:self.XComC.shape[1]]
        ypos = ypos
        xcut = xpos[cutstart[0]:self.XComC.shape[0], cutstart[1]:self.XComC.shape[1]] - cutstart[1]
        ycut = np.flipud(ypos[cutstart[0]:self.XComC.shape[0], cutstart[1]:self.XComC.shape[1]]) - cutstart[0]
        dx = self.XComC[cutstart[0]:self.XComC.shape[0], cutstart[1]:self.XComC.shape[1]]
        dy = self.YComC[cutstart[0]:self.XComC.shape[0], cutstart[1]:self.XComC.shape[1]]
        cc_cut = cc_color[cutstart[0]:self.XComC.shape[0], cutstart[1]:self.XComC.shape[1], :]
        ...
        plt.gca().add_artist(scalebar)
        plt.axis('off')
        at = mploff.AnchoredText('Center of Mass Shift', prop=dict(size=fontsize), 
                                 frameon=True, loc='lower left')
        at.patch.set_boxstyle("round, pad=0., rounding_size=0.2")
        plt.gca().add_artist(at)
